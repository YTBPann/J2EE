...
public class ExchangeService {
    ...
    public ExchangeRequest createRequest(Long requesterId, Long ownerId, Long offeredProductId, 
        ...
        if (!"AVAILABLE".equals(offeredProduct.getStatus()) || !"AVAILABLE".equals(requestedProduct.getStatus())) {
            throw new RuntimeException("Only admin-approved available products can be exchanged");
        }
    ...
    public ExchangeRequest completeRequest(Long requestId) {
        ExchangeRequest request = exchangeRequestRepository.findById(requestId)
            .orElseThrow(() -> new RuntimeException("Exchange request not found"));
        int currentLevel = request.getDeliveryConfirmationLevel() == null ? 0 : request.getDeliveryConfirmationLevel();
        if (currentLevel < 4) {
